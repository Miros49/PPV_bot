GTA5_PROJECTS = [
    'GTA5RP', 'Majestic', 'Grand RP GTA5',
    'Radmir GTA5', 'Arizona RP GTA5', 'RMRP GTA5'
]

OTHER_PROJECTS = [
    'Black Russia', 'Arizona RP', 'Radmir RP',
    'NEXTRP', 'Evolve RP'
]


PROJECTS = {
    'gta5': GTA5_PROJECTS,
    'other': OTHER_PROJECTS
}






# GTA5
#
# GTA5RP (сервера есть)
# Majestic RP (сервера есть)
# Radmir RP GTA5 (сервера есть + МОЙ ДОМ)
# RMRP (есть сервера)
# Arizona V  (есть сервера)
# Grand RP (Оставить только русские (2 сервера))
# SMOTRArage (Один сервер у проекта)
# Quant RP (Один сервер у проекта)
#
# SAMP/MOBILE/CRMP
# BLACK RUSSIA (есть сервера + 76. Chita 77. Kostroma)
# ARIZONA RP (+ 28. Christmas, 29. Mirage, 30. Love)
# GRAND MOBILE (1-33)
# МАТРЕШКА RP (1-30)
# HASSLE ONLINE (1-21)
# Radmir RP (1-21) (Ярлык Hassle Online, тобиш HASSLE ONLINE и Radmir RP одинаковые, если создается ордер на хасле, то он автоматически и на хасле и на радмире рп)
# RODINA RP - Центральный округ, Южный округ, Северный округ, Восточный округ, Западный округ, Приморский округ, Федеральный округ
# Amazing RP (Red, Yellow, Green, Azure, Silver, Rose, Black)
# NEXTRP (есть сервера)
